...
def is_contained(a_lo: int, a_hi: int, b_lo: int, b_hi: int) -> bool:
    if (a_lo >= b_lo and a_hi <= b_hi) or (b_lo >= a_lo and b_hi <= a_hi):
        return True
    return False

results: list = [is_contained(int(pair[0][0]), int(pair[0][1]), int(pair[1][0]), int(pair[1][1])) for pair in pairs]
