...
def get_desired_char(op, directive):
    if directive == 'A': return op[2][1]
    if directive == 'B': return op[1]
    if directive == 'C': return op[0][1]


matches = [(match[0], get_desired_char(get_item(match[0]), match[1])) for match in normalized_matches]
...
points = 0
for match_points in [get_points(match) for match in matches]:
    points += match_points
